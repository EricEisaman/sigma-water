...
fn main(input : FragmentInputs) -> FragmentOutputs {
  ...
  private createOceanWavesShader(): ShaderMaterial {
    if (!this.scene) throw new Error('Scene not initialized');
...
`;
    ...
    try {
      const oceanWavesShader = new ShaderMaterial(
        `oceanWaves_${Date.now()}`,
        this.scene,
        {
          vertexSource: vertexCode,
          fragmentSource: fragmentCode,
      ...
      oceanWavesShader.transparencyMode = Material.MATERIAL_OPAQUE;
      oceanWavesShader.alpha = 1.0;
      oceanWavesShader.backFaceCulling = false;
      oceanWavesShader.wireframe = false;

      console.log('✅ Ocean Waves shader created successfully');
      return oceanWavesShader;
    } catch (error) {
      console.error('❌ Failed to create oceanWaves shader:', error);
      throw error;
    }
  }
