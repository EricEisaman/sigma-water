...
import { Scene, Mesh, ShaderMaterial, Material } from '@babylonjs/core';
...
export class ShaderContext {
  ...
  public initialize(): void {
    try {
      ...
      this.state.material = new ShaderMaterial(
        ...
        this.scene,
        {
          vertexSource: this.config.vertexCode,
          fragmentSource: this.config.fragmentCode,
        },
        {
      ...
      this.state.material.transparencyMode = Material.MATERIAL_OPAQUE;
      this.state.material.alpha = 1.0;
      this.state.material.backFaceCulling = false;
      this.state.material.wireframe = false;
  ...
  public activate(mesh: Mesh): void {
    ...
    try {
      ...
      if (!this.state.material) {
        throw new Error('Shader material not initialized');
      }
  ...
  public setUniform(name: string, value: any): void {
    if (!this.state.material) {
      console.warn(`⚠️ Cannot set uniform: material not initialized`);
      return;
    }
