...
export class ShaderManager {
  ...
  /**
   * Dispose material for the currently active shader context.
   * Used when mesh is reused and material must be recreated on switch.
   */
  public disposeActiveMaterial(): void {
    if (!this.activeContextId) return;
    const activeContext = this.contexts.get(this.activeContextId);
    activeContext?.disposeMaterial();
  }
