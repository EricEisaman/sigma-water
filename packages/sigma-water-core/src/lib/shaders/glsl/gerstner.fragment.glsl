...
void main(void) {
  ...
  vec3 troughColor = vec3(0.01, 0.05, 0.11);
  vec3 bodyColor = vec3(0.03, 0.19, 0.31);
  vec3 crestColor = vec3(0.12, 0.42, 0.56);
  vec3 waterColor = mix(crestColor, troughColor, depthLerp);
  ...
  float specularRaw = pow(max(dot(normal, halfVec), 0.0), specPower) * specNorm * (0.08 + specularStrength * 0.42);
  float specularWide = pow(max(dot(normal, halfVec), 0.0), mix(24.0, 72.0, gloss)) * (0.02 + specularStrength * 0.08);
  float specular = min(specularRaw + specularWide, 0.22);
  ...
  // CAUSTICS
  float causticWave = sin(dot(vPositionW.xz, windDir) * (freq * 1.1) - time * 0.35) * 0.5 + 0.5;
  float caustic = (1.0 - depthLerp) * causticWave * max(causticIntensity, 0.0) * 0.08;
  waterColor += vec3(0.04, 0.09, 0.08) * caustic;
  
  // SKY REFLECTION
  vec3 scatter = vec3(0.0, 0.09, 0.12) * pow(1.0 - ndv, 1.7) * (0.45 + amp * 0.55);
  float skyMix = clamp(skyReflectionMix, 0.0, 0.9);
  vec3 reflectionDir = reflect(-viewDir, normal);
  vec3 skyColor = getSkyColor(reflectionDir);
  vec3 reflectionTint = mix(vec3(0.72, 0.84, 0.95), skyColor, skyMix);
  vec3 lit = waterColor * (0.18 + ndl * 0.82) + scatter;
  vec3 reflected = min(reflectionTint * fresnel * (0.14 + ndl * 0.08), vec3(0.38, 0.42, 0.48));
  float foamTone = smoothstep(0.2, 0.86, detailedNoise);
  vec3 foamColor = mix(vec3(0.79, 0.86, 0.92), vec3(0.94, 0.97, 1.0), foamTone) * foam;
  
  // FINAL COLOR
  vec3 horizonGlow = vec3(0.04, 0.08, 0.12) * pow(1.0 - ndv, 2.2);
