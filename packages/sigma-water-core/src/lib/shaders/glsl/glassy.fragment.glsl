...
void main(void) {
  ...
  // Glassy colors: highly reflective, light
  vec3 shallowColor = vec3(0.2, 0.52, 0.65);
  vec3 deepColor = vec3(0.08, 0.25, 0.42);
  vec3 waterColor = mix(shallowColor, deepColor, depthLerp);
  waterColor = mix(waterColor, vec3(0.12, 0.35, 0.5), clamp(ndl * 0.45 + 0.2, 0.0, 1.0));
  ...
  float specularRaw = pow(max(dot(normal, halfVec), 0.0), specPower) * specNorm * (0.16 + specularStrength * 0.68);
  float specularWide = pow(max(dot(normal, halfVec), 0.0), mix(24.0, 72.0, gloss)) * (0.06 + specularStrength * 0.18);
  float specular = min(specularRaw + specularWide, 0.32);
  ...
  vec3 scatter = vec3(0.05, 0.18, 0.22) * pow(1.0 - ndv, 1.7) * (0.45 + amp * 0.55);
  float skyMix = clamp(skyReflectionMix, 0.0, 0.9);
  vec3 lit = waterColor * (0.2 + ndl * 0.8) + scatter;
  vec3 reflected = vec3(0.8, 0.9, 0.98) * fresnel * (0.26 + ndl * 0.14);
  float foamTone = smoothstep(0.2, 0.86, detailedNoise);
  vec3 foamColor = mix(vec3(0.88, 0.94, 0.99), vec3(0.98, 1.0, 1.0), foamTone) * foam;
  
  vec3 horizonGlow = vec3(0.15, 0.22, 0.28) * pow(1.0 - ndv, 2.2);
