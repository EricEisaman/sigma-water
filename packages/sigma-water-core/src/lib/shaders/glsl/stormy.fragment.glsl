...
void main(void) {
  ...
  // Dark stormy colors
  vec3 troughColor = vec3(0.0, 0.02, 0.05);
  vec3 bodyColor = vec3(0.01, 0.08, 0.15);
  vec3 crestColor = vec3(0.08, 0.25, 0.38);
  ...
  float specularRaw = pow(max(dot(normal, halfVec), 0.0), specPower) * specNorm * (0.06 + specularStrength * 0.35);
  float specularWide = pow(max(dot(normal, halfVec), 0.0), mix(24.0, 72.0, gloss)) * (0.01 + specularStrength * 0.05);
  float specular = min(specularRaw + specularWide, 0.18);
  ...
  vec3 scatter = vec3(0.0, 0.05, 0.08) * pow(1.0 - ndv, 1.7) * (0.45 + amp * 0.55);
  float skyMix = clamp(skyReflectionMix, 0.0, 0.9);
  vec3 lit = waterColor * (0.15 + ndl * 0.85) + scatter;
  vec3 reflected = vec3(0.55, 0.65, 0.75) * fresnel * (0.12 + ndl * 0.08);
  float foamTone = smoothstep(0.2, 0.86, detailedNoise);
  vec3 foamColor = mix(vec3(0.75, 0.82, 0.88), vec3(0.92, 0.95, 0.98), foamTone) * foam;
  
  vec3 horizonGlow = vec3(0.02, 0.05, 0.08) * pow(1.0 - ndv, 2.2);
